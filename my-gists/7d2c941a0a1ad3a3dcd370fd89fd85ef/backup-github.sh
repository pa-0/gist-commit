...
REPOLIST=`check curl --silent -u $GHBU_UNAME:$GHBU_PASSWD ${GHBU_API}/user/repos -q | check grep "\"full_name\"" | check awk -F': "' '{print $2}' | check sed -e 's/",//g'`
...
for REPO in $REPOLIST; do
   ...
   check ${GHBU_GIT_CLONE_CMD}${REPO}.git ${GHBU_BACKUP_DIR}/${REPO}-${TSTAMP}.git && tgz ${GHBU_BACKUP_DIR}/${REPO}-${TSTAMP}.git

   $GHBU_SILENT || echo "Backing up ${REPO}.wiki (if any)"
   ${GHBU_GIT_CLONE_CMD}${REPO}/.wiki.git ${GHBU_BACKUP_DIR}/${REPO}.wiki-${TSTAMP}.git 2>/dev/null && tgz ${GHBU_BACKUP_DIR}/${REPO}.wiki-${TSTAMP}.git

   $GHBU_SILENT || echo "Backing up ${REPO} issues"
   check curl --silent -u $GHBU_UNAME:$GHBU_PASSWD ${GHBU_API}/repos/${REPO}/issues -q > ${GHBU_BACKUP_DIR}/${REPO}.issues-${TSTAMP} && tgz ${GHBU_BACKUP_DIR}/${REPO}.issues-${TSTAMP}
done
