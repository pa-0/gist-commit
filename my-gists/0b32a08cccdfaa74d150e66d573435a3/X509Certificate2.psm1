...
.Parameter Password
A password to encrypt the private key. Valid only with Pkcs8 or PrivateKey.

.Parameter Certificate
The certificate to save.

.Example
$certificate | Export-X509Certificate2 key.pem -Type Pkcs8 -Password (Read-Host -Prompt Password -AsSecureString)

Format the $certificate using PKCS8 encrypted with a password read from the host.
#>
function Export-X509Certificate2 {
    ...
    if ($Type -in 'Pfx', 'Pkcs12') {
        ...
        Format-X509Certificate2 @PSBoundParameters | Set-Content $Path -Encoding ASCII
    }
}
...
.Example
$certificate | Format-X509Certificate2 -Type Pkcs8 -Password (Read-Host -Prompt Password -AsSecureString)
...
"@
    ...
    if ($Type -in 'Pkcs1', 'Pkcs8', 'PrivateKey') {
        $privateKey = [System.Security.Cryptography.X509Certificates.RSACertificateExtensions]::GetRSAPrivateKey($Certificate)
    }

    switch ($Type) {
        'Certificate' { ConvertTo-Pem 'CERTIFICATE' $Certificate.RawData }
        'CertificateBase64' { [Convert]::ToBase64String($Certificate.RawData, 'InsertLineBreaks') }
        'Pkcs12Base64' { [Convert]::ToBase64String($Certificate.Export([X509ContentType]::Pfx), 'InsertLineBreaks') }
        'Pkcs1' { ConvertTo-Pem 'RSA PRIVATE KEY' $privateKey.ExportRSAPrivateKey() }
        { $_ -in 'Pkcs8', 'PrivateKey' -and $Password } {
            $pbe = [PbeParameters]::new(
                'Aes256Cbc',
                [HashAlgorithmName]::SHA256,
                100000
            )
            [char[]] $pw = ConvertFrom-SecureString $Password -AsPlainText
            ConvertTo-Pem 'ENCRYPTED PRIVATE KEY' $privateKey.ExportEncryptedPkcs8PrivateKey($pw, $pbe)
            break
        }
        { $_ -in 'Pkcs8', 'PrivateKey' } { ConvertTo-Pem 'PRIVATE KEY' $privateKey.ExportPkcs8PrivateKey() }
    }
}
